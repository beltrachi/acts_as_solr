  ...
  module InstanceMethods
    ...
    def to_solr_doc
      ...
      configuration[:solr_fields].each do |field_name, options|
        ...
        prepare_and_add_field_to_doc( field_name, options, doc )
      end
    ...
    def prepare_and_add_field_to_doc( field_name, options, doc )
      ...
      if field_name.to_s != self.class.primary_key and field_name.to_s != "type"
        add_field_to_doc( field_type, options, field_name, doc, value, solr_name )
      end
    end
    
    def add_field_to_doc( field_type, options, field_name, doc, value, solr_name)
      ...
      if options[:sliced]
        add_slices_to_doc( field_type, options.dup, field_name, doc, value, solr_name)
      end
    ...
    # Adds the slices of the field to the doc
