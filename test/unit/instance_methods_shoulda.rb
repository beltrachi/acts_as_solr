  ...
  context "With a Solr record instance" do
    ...
    context "when converting an instance to a solr document" do
      ...
      context "with indexed fields" do
        ...
        context "when a range double sliced twice field configured" do
          setup do
            @instance.configuration[:solr_fields].merge!( 
              { :lat => { :type => :range_double, :sliced => 2 } }  )
            @instance.stubs(:get_solr_field_type).with(:range_double).returns('rd')
            @instance.stubs(:get_solr_field_type).with("rd").returns('rd')
            @instance.stubs(:get_solr_field_type).with(:range_integer).returns('ri')
            @instance.stubs(:get_solr_field_type).with("ri").returns('ri')
          ...
          should "save the double field and its int field" do
            ...
            doc = @instance.to_solr_doc
            assert_equal doc[:lat_rd], "12.123456789"
            assert_equal doc[:lat_4d_rd], "12.1234"
            assert_equal doc[:lat_ri], "12"
          end
          ...
          should "save the double field and its int truncated not rounded field" do
            ...
            doc = @instance.to_solr_doc
            assert_equal doc[:lat_rd], "12.999999"
            assert_equal doc[:lat_4d_rd], "12.9999"
            assert_equal doc[:lat_ri], "12"
          end
          ...
          should "lat is nill so no field saved" do
            ...
            @instance.stubs(:set_value_if_nil).returns("")
            doc = @instance.to_solr_doc
            assert_nil doc[:lat_4d_rd]
            assert_nil doc[:lat_rd]
            assert_nil doc[:lat_ri]
          end
        end

        context "when a double range sliced N times fields configured" do
          setup do
            @instance.configuration[:solr_fields].merge!(
          ...
          should "save the double field and its int field" do
            @instance.stubs(:lat_for_solr).returns( "12.123456789812345" )
            doc = @instance.to_solr_doc
            assert_equal doc[:lat_rd], "12.123456789812345"
            assert_equal doc[:lat_4d_rd], "12.1234"
            assert_equal doc[:lat_10d_rd], "12.1234567898"
            assert_equal doc[:lat_ri], "12"
          end

          should "save the double field and its int truncated not rounded field" do
            @instance.stubs(:lat_for_solr).returns( "12.9999999999999999" )
            doc = @instance.to_solr_doc
            assert_equal doc[:lat_rd], "12.9999999999999999"
            assert_equal "12.9999999999", doc[:lat_10d_rd]
            assert_equal doc[:lat_4d_rd], "12.9999"
            assert_equal doc[:lat_ri], "12"
          end

          should "save the negative double field and its int field" do
            @instance.stubs(:lat_for_solr).returns( "-12.123456789812345678" )
            doc = @instance.to_solr_doc
            assert_equal "-12.123456789812345678", doc[:lat_rd]
            assert_equal doc[:lat_10d_rd], "-12.1234567898"
            assert_equal doc[:lat_4d_rd], "-12.1234"
            assert_equal doc[:lat_ri], "-12"
          end

          should "save a short double field and its int field" do
            @instance.stubs(:lat_for_solr).returns( -12.1 )
            doc = @instance.to_solr_doc
            assert_equal doc[:lat_rd], "-12.1"
            assert_equal doc[:lat_10d_rd], "-12.1"
            assert_equal doc[:lat_4d_rd], "-12.1"
            assert_equal doc[:lat_ri], "-12"
          end
